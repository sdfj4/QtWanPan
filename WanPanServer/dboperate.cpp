...
QStringList DBOperate::handleFlushFriend(const char *name)
{
    ...
    // 获取请求方name对应的id
    QString strQuery = QString("select name from userInfo where online = 1 and id in (select id from friendinfo where friendId = (select id from userInfo where name = \'%1\'))").arg(name);
    QSqlQuery query;
    query.exec(strQuery);
    while(query.next())
    {
        strFriendList.append(query.value(0).toString());
        // qDebug() << "好友信息 " << query.value(0).toString();
        // qDebug() << strFriendList;
    }
    strQuery = QString("select name from userInfo where online = 1 and id in (select friendId from friendinfo where id = (select id from userInfo where name = \'%1\'))").arg(name);
    query.exec(strQuery);
    while(query.next())
    {
        strFriendList.append(query.value(0).toString());

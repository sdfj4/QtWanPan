...
MytcpSocket::MytcpSocket()
{
    ...
    connect(this,SIGNAL(readyRead()),this,SLOT(recMsg()));
...
PDU* handleFlushFriendRequest(PDU* pdu)
{
    ...
    uint uiMsgLen = strList.size()*32; // 好友名字
    qDebug()<<"好友列表长度"<<uiMsgLen;
    PDU* resPdu = mkPDU(uiMsgLen);
    resPdu -> uiMsgType = ENUM_MSG_TYPE_FLUSH_FRIEND_RESPOND;
    for(int i = 0; i < strList.size(); i++ )
    {
        memcpy((char*)(resPdu -> caMsg) + i*32, strList.at(i).toStdString().c_str(), 32);
        qDebug()<<"好友长度"<<strList.size();
        qDebug()<<"查询好友"<<(char*)(resPdu -> caMsg) + i*32;
...
PDU* handlePrivateChatRequest(PDU* pdu)
{
    ...
    strncpy(chatedName, pdu -> caData + 32, 32);
